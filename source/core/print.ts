...
import { PrintTargetType, __SYMBOL_MESSAGE__, __SYMBOL_ARRAY__ } from './messageType';
 ...
 */
export function toPrintStyle(style: StyleKey | StyleKey[] | StandardLonghandProperties): __SYMBOL_MESSAGE__<string> {
  if (typeof style === 'string') return new __SYMBOL_MESSAGE__(keyToAnsi[style] ?? '', DEFINE_MESSAGE.PRINTER_MESSAGE_ANSI_STYLE_FLAG);
  if (Array.isArray(style)) return new __SYMBOL_MESSAGE__(style.reduce((pre, cur) => pre + (keyToAnsi[cur] ?? ''), ''), DEFINE_MESSAGE.PRINTER_MESSAGE_ANSI_STYLE_FLAG);
...
/**
 * 获得最终格式化输出数组
 * @param arr 需要格式化输出的信息
 * @returns 返回格式化后的数组信息, 并给这个格式化数组加上唯一标识, 以供 print 函数的识别
 */
export const toPrintArr = <T>(arr: T[]): __SYMBOL_MESSAGE__<T[]> => new __SYMBOL_MESSAGE__(arr, DEFINE_MESSAGE.PRINTER_MESSAGE_ARR_FLAG);
 ...
 * @returns 返回格式化输出数组
 */
export function toColor<T>(style: StyleKey | StyleKey[] | StandardLonghandProperties, ...message: T[]): __SYMBOL_ARRAY__<[string, ...((string | T | __SYMBOL_MESSAGE__<string>)[])]> {
...
export const isPrintStyleMessageArr = <T>(target: T | __SYMBOL_ARRAY__<[string, ...((string | __SYMBOL_MESSAGE__<string>)[])]>): target is __SYMBOL_ARRAY__<[string, ...((string | __SYMBOL_MESSAGE__<string>)[])]> => {
  return target instanceof __SYMBOL_ARRAY__;
}

export const isPrintStyleMessage = <T>(target: T | __SYMBOL_MESSAGE__<string>): target is __SYMBOL_MESSAGE__<string> => {
  return target instanceof __SYMBOL_MESSAGE__;
}
